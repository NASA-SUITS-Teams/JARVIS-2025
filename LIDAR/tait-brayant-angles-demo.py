...
def process_lidar_readings(lidar_readings, rover_position, rover_angles, sensor_data):
  """
...
def main():
  # Set up the rover's global pose.
  ...
  # We'll simulate a constant reading of 500 cm (5 meters) for each sensor.
  constant_readings = [500] * 13  # 13 sensors
  
  try:
    while True:
      # Process the constant LIDAR readings.
      points = process_lidar_readings(constant_readings, rover_position, rover_angles, sensor_data)
      print("Points generated:", points)  # Debug: ensure points are non-empty
      
      # Update the point cloud.
      pcd.points = o3d.utility.Vector3dVector(np.array(points))
      
      # Optionally update the camera view
      ctr = vis.get_view_control()
      ctr.set_zoom(0.5)
      ctr.set_front([0.0, -1.0, 0.0])
      ctr.set_lookat([rover_position[0], rover_position[1], rover_position[2]])
      ...
      print("Stopping LIDAR data acquisition...")
  finally:
      vis.destroy_window()
